...
int main(){
     ...
     * */
    vector<string> datasetNames = retrieveAdHocFolder();
    ...
    for(const string& dataset: datasetNames) {
        ...
        /**
         * Initialization of the Page Rank Matrix data structure.
         * */
        ...
        for (int thread = 1; thread <= maxNumThread; thread++) {
            ...
            for (int round = 0; round < maxNumRound; round++) {
                ...
                vector<float> v = probabilityDistributionVector(matrix.size());
                float beta = 0.85;
                float teleport_constant = (float) (1.0 - beta) / (float) matrix.size();
                vector<float> res(matrix.size(), 0.0);
                ...
                while(iter < 50) {
                    ...
                    pageRankMatrix(matrix, v, res, beta, teleport_constant, thread, threads);
                    v = res;
                    iter++;
                }
     ...
     * Saving data.
     * */
    savingIntoCSV("datasets/executionTimesMatrix.csv", allTimePoints);
    savingPairsIntoCSV("datasets/threadTimesMatrix.csv", timeThreads);
